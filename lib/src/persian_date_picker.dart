...
class _DatePickerHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final MaterialLocalizations localizations = PersianMaterialLocalizations();
...
/// [showPersianDatePicker], which creates a date picker dialog.
///
/// See also:
///
///  * [showPersianDatePicker], which shows a dialog that contains a material design
///    date picker.
...
class DayPicker extends StatelessWidget {
  ...
  int _computeFirstDayOffset(int year, int month) {
    // 0-based day of week, with 0 representing Saturday.
    final int weekdayFromSaturday = Jalali(year, month).weekDay - 1;
    return weekdayFromSaturday % 7;
  }
  ...
  Widget build(BuildContext context) {
    final ThemeData themeData = Theme.of(context);
    final MaterialLocalizations localizations = PersianMaterialLocalizations();
    final persianDisplayedMonth = Jalali.fromDateTime(displayedMonth);
    final persianSelectedDate = Jalali.fromDateTime(selectedDate);
    final persianCurrentDate = Jalali.fromDateTime(currentDate);
    final int year = persianDisplayedMonth.year;
    final int month = persianDisplayedMonth.month;
    ...
    for (int i = 0; true; i += 1) {
      ...
      if (day < 1) {
        ...
        final persianDayToBuild = Jalali(year, month, day);
        final DateTime dayToBuild = persianDayToBuild.toDateTime();
        ...
        final bool isSelectedDay = persianSelectedDate.year == year &&
            persianSelectedDate.month == month &&
            persianSelectedDate.day == day;
        if (isSelectedDay) {
        ...
        } else if (persianCurrentDate.year == year &&
            persianCurrentDate.month == month &&
            persianCurrentDate.day == day) {
...
class _MonthPickerState extends State<MonthPicker>
  ...
  void didChangeDependencies() {
    ...
    localizations = PersianMaterialLocalizations();
  ...
  static int _monthDelta(DateTime startDate, DateTime endDate) {
    final s = Jalali.fromDateTime(startDate);
    final e = Jalali.fromDateTime(endDate);
    return (e.year - s.year) * 12 + e.month - s.month;
  }

  /// Add months to a month truncated date.
  DateTime _addMonthsToMonthDate(DateTime monthDate, int monthsToAdd) {
    final j = Jalali.fromDateTime(monthDate);
    return Jalali(j.year + monthsToAdd ~/ 12, j.month + monthsToAdd % 12)
        .toDateTime();
  }
...
class _YearPickerState extends State<YearPicker> {
  ...
  Widget build(BuildContext context) {
    ...
    final persianSelectedDate = Jalali.fromDateTime(widget.selectedDate);
    final persianFirstDate = Jalali.fromDateTime(widget.firstDate);
    final persianLastDate = Jalali.fromDateTime(widget.lastDate);
    return ListView.builder(
      dragStartBehavior: widget.dragStartBehavior,
      controller: scrollController,
      itemExtent: _itemExtent,
      itemCount: persianLastDate.year - persianFirstDate.year + 1,
      itemBuilder: (BuildContext context, int index) {
        final int year = persianFirstDate.year + index;
        final bool isSelected = year == persianSelectedDate.year;
        ...
        return InkWell(
          ...
          onTap: () {
            final selected = Jalali(
                year, persianSelectedDate.month, persianSelectedDate.day);
            widget.onChanged(selected.toDateTime());
          },
          child: Center(
            child: Semantics(
              selected: isSelected,
...
///
/// ```dart
/// Future<DateTime> selectedDate = showPersianDatePicker(
///   context: context,
